<script setup>
    ...
    const remainPrizes = computed(() =>
        store.prizes.reduce((sum, p) => sum + (p.count - p.drawn), 0)
    );
    const isAllFinished = computed(() => remainPrizes.value <= 0);

    const currentPrize = computed(() => store.prizes.find((p) => p.id === store.currentPrizeId));
    const isCurrentPrizeFinished = computed(() => {
        if (!currentPrize.value) return true;
    ...
    const stopRolling = () => {
        ...
        if (store.settings.drawMode === "selective") {
            // 执行指定奖项抽奖
            winners = store.drawPrize(store.currentPrizeId, drawCount.value);
        } else {
            // 执行混合抽奖
            winners = store.drawMixed(drawCount.value);
        }
...
<template>
    <div
        class="lottery-page"
        :style="{
            backgroundImage: store.settings.bgImage ? `url(${store.settings.bgImage})` : undefined
        }">
        <!-- 顶部 -->
        <header class="header">
        ...
        <div class="main-layout">
                ...
                <div class="prize-list">
                    <div
                        ...
                        :class="{
                            ...
                            active:
                                store.settings.drawMode === 'selective' &&
                                store.currentPrizeId === prize.id,
                            clickable: store.settings.drawMode === 'selective'
                        }"
                        @click="store.settings.drawMode === 'selective' && selectPrize(prize.id)">
                        ...
                        <div class="progress-bar">
                            <div
                                class="progress-fill"
                                :style="{ width: `${(prize.drawn / prize.count) * 100}%` }"></div>
                        </div>
                    </div>
                </div>
            </aside>
            ...
            <main class="center-panel">
                ...
                <div
                    class="prize-info"
                    v-if="store.settings.drawMode === 'selective' && currentPrize">
                    <div class="mixed-info">
                        <icon-trophy size="36" style="color: gold; margin-bottom: 5px" />
                        <h2 class="prize-name">{{ currentPrize.name }}</h2>
                        <div class="pool-stat">
                            该奖项剩余:
                            <span class="num">{{ currentPrize.count - currentPrize.drawn }}</span> /
                            <span class="total">{{ currentPrize.count }}</span>
                        </div>
                    </div>
                </div>
                ...
                <div
                    class="prize-info"
                    v-else-if="store.settings.drawMode === 'mixed' && !isAllFinished">
                    <div class="mixed-info">
                        <icon-trophy size="36" style="color: gold; margin-bottom: 5px" />
                        <h2 class="prize-name">年会大抽奖 (全奖池)</h2>
                        <div class="pool-stat">
                            奖池总剩余: <span class="num">{{ remainPrizes }}</span> /
                            <span class="total">{{ totalPrizes }}</span>
                        </div>
                    </div>
                </div>

                <!-- 所有奖项抽完的提示 -->
                <div class="prize-info" v-else-if="isAllFinished">
                ...
                <div class="winners-list">
                    <div v-for="winner in winnersList" :key="winner.id" class="winner-row">
                        ...
                        <div class="winner-row-prize">
                            <a-tag :color="winner.prizeColor || 'gold'" size="small">{{
                                winner.prizeName
                            }}</a-tag>
                        </div>
                    </div>
                    <div v-if="winnersList.length === 0" class="empty-tip">暂无中奖记录</div>
                </div>
                <!-- 查看全部按钮 -->
